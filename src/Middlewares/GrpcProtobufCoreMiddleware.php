...
use FrockDev\ToolsForLaravel\Routes\GrpcDispatcherFactory;
...
class GrpcProtobufCoreMiddleware extends HttpCoreMiddleware
{
    /**
     * @var null|Protocol
     */
    ...
    {
        if ($this->protocol) {
            $factory = make(GrpcDispatcherFactory::class, [
                'pathGenerator' => $this->protocol->getPathGenerator(),
            ]);
            return $factory->getDispatcher($serverName);
        } else {
            $factory = $this->container->get(GrpcDispatcherFactory::class);
            return $factory->getDispatcher($serverName);
        }
    }

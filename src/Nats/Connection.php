...
//        }
    ...
    private function handlePONG($line)
    {
        if ($this->debug === true) {
            printf('>>fromServer>> %s', $line);
        }
//        $parts   = explode(' ', $line);
//        if (count($parts) < 4) {
//            throw new \Exception(sprintf('Invalid MSG data received: %s', $line));
...
//        }
    ...
    public function wait($quantity = 0)
    {
        ...
        while ($count<$quantity) {
            ...
            if (!(is_resource($this->streamSocket) === true && feof($this->streamSocket) === false && empty($info['timed_out']) === true)) {
                $this->close();
                return $this;
            }
            $this->sendPing();
